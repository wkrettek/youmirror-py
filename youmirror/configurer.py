...
valid_options = {
    ...
    "playlist",
    "single",
}  # These are the valid global options
...
def remove_yt(yt_string: str, id: str, config: dict) -> dict:
    ...
    try:
        ...
        yt_section = config[
            yt_string
        ]  # Get the section "channel", "playlist", "single"
        if id in yt_section:  # If it's there
            del config[yt_string][id]  # Delete it
        return config
    except Exception as e:
        logging.exception(
            "Could not remove %s, %s from config due to %s", yt_string, id, e
        )
...
def new_config(config_path: Path, root: str) -> Path:
    ...
    try:
        if not config_path.is_file():  # Only if there isn't one already
            ...
            from youmirror.template import \
                template  # Import the template dictionary
